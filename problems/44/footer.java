...
public class Main {
    public static void main(String args[]) throws Exception {
        ...
        for (int i = 1; i <= cases; i++) {
            ...
            int B = Integer.parseInt(inputLine.substring(lastSpace + 1));
            inputLine = inputLine.substring(0, lastSpace);

            int secondLastSpace = inputLine.lastIndexOf(" ");
            int A = Integer.parseInt(inputLine.substring(secondLastSpace + 1));
            inputLine = inputLine.substring(0, secondLastSpace);

            BinaryTree tree = new BinaryTree();
            tree.deserializeTree(inputLine);
            Solution s = new Solution();
            int result = s.lca(tree.root, A, B);
